// generated from rosidl_typesupport_c/resource/idl__type_support.cpp.em
// with input from amr_v4_msgs_srvs:srv/Testserv.idl
// generated code does not contain a copyright notice

#include "cstddef"
#include "rosidl_runtime_c/message_type_support_struct.h"
#include "amr_v4_msgs_srvs/srv/detail/testserv__struct.h"
#include "amr_v4_msgs_srvs/srv/detail/testserv__type_support.h"
#include "amr_v4_msgs_srvs/srv/detail/testserv__functions.h"
#include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/message_type_support_dispatch.h"
#include "rosidl_typesupport_c/type_support_map.h"
#include "rosidl_typesupport_c/visibility_control.h"
#include "rosidl_typesupport_interface/macros.h"

namespace amr_v4_msgs_srvs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _Testserv_Request_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _Testserv_Request_type_support_ids_t;

static const _Testserv_Request_type_support_ids_t _Testserv_Request_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _Testserv_Request_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _Testserv_Request_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _Testserv_Request_type_support_symbol_names_t _Testserv_Request_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, amr_v4_msgs_srvs, srv, Testserv_Request)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, amr_v4_msgs_srvs, srv, Testserv_Request)),
  }
};

typedef struct _Testserv_Request_type_support_data_t
{
  void * data[2];
} _Testserv_Request_type_support_data_t;

static _Testserv_Request_type_support_data_t _Testserv_Request_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _Testserv_Request_message_typesupport_map = {
  2,
  "amr_v4_msgs_srvs",
  &_Testserv_Request_message_typesupport_ids.typesupport_identifier[0],
  &_Testserv_Request_message_typesupport_symbol_names.symbol_name[0],
  &_Testserv_Request_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t Testserv_Request_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_Testserv_Request_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &amr_v4_msgs_srvs__srv__Testserv_Request__get_type_hash,
  &amr_v4_msgs_srvs__srv__Testserv_Request__get_type_description,
  &amr_v4_msgs_srvs__srv__Testserv_Request__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace amr_v4_msgs_srvs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, amr_v4_msgs_srvs, srv, Testserv_Request)() {
  return &::amr_v4_msgs_srvs::srv::rosidl_typesupport_c::Testserv_Request_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "amr_v4_msgs_srvs/srv/detail/testserv__struct.h"
// already included above
// #include "amr_v4_msgs_srvs/srv/detail/testserv__type_support.h"
// already included above
// #include "amr_v4_msgs_srvs/srv/detail/testserv__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace amr_v4_msgs_srvs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _Testserv_Response_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _Testserv_Response_type_support_ids_t;

static const _Testserv_Response_type_support_ids_t _Testserv_Response_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _Testserv_Response_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _Testserv_Response_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _Testserv_Response_type_support_symbol_names_t _Testserv_Response_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, amr_v4_msgs_srvs, srv, Testserv_Response)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, amr_v4_msgs_srvs, srv, Testserv_Response)),
  }
};

typedef struct _Testserv_Response_type_support_data_t
{
  void * data[2];
} _Testserv_Response_type_support_data_t;

static _Testserv_Response_type_support_data_t _Testserv_Response_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _Testserv_Response_message_typesupport_map = {
  2,
  "amr_v4_msgs_srvs",
  &_Testserv_Response_message_typesupport_ids.typesupport_identifier[0],
  &_Testserv_Response_message_typesupport_symbol_names.symbol_name[0],
  &_Testserv_Response_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t Testserv_Response_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_Testserv_Response_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &amr_v4_msgs_srvs__srv__Testserv_Response__get_type_hash,
  &amr_v4_msgs_srvs__srv__Testserv_Response__get_type_description,
  &amr_v4_msgs_srvs__srv__Testserv_Response__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace amr_v4_msgs_srvs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, amr_v4_msgs_srvs, srv, Testserv_Response)() {
  return &::amr_v4_msgs_srvs::srv::rosidl_typesupport_c::Testserv_Response_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "amr_v4_msgs_srvs/srv/detail/testserv__struct.h"
// already included above
// #include "amr_v4_msgs_srvs/srv/detail/testserv__type_support.h"
// already included above
// #include "amr_v4_msgs_srvs/srv/detail/testserv__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace amr_v4_msgs_srvs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _Testserv_Event_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _Testserv_Event_type_support_ids_t;

static const _Testserv_Event_type_support_ids_t _Testserv_Event_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _Testserv_Event_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _Testserv_Event_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _Testserv_Event_type_support_symbol_names_t _Testserv_Event_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, amr_v4_msgs_srvs, srv, Testserv_Event)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, amr_v4_msgs_srvs, srv, Testserv_Event)),
  }
};

typedef struct _Testserv_Event_type_support_data_t
{
  void * data[2];
} _Testserv_Event_type_support_data_t;

static _Testserv_Event_type_support_data_t _Testserv_Event_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _Testserv_Event_message_typesupport_map = {
  2,
  "amr_v4_msgs_srvs",
  &_Testserv_Event_message_typesupport_ids.typesupport_identifier[0],
  &_Testserv_Event_message_typesupport_symbol_names.symbol_name[0],
  &_Testserv_Event_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t Testserv_Event_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_Testserv_Event_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &amr_v4_msgs_srvs__srv__Testserv_Event__get_type_hash,
  &amr_v4_msgs_srvs__srv__Testserv_Event__get_type_description,
  &amr_v4_msgs_srvs__srv__Testserv_Event__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace amr_v4_msgs_srvs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, amr_v4_msgs_srvs, srv, Testserv_Event)() {
  return &::amr_v4_msgs_srvs::srv::rosidl_typesupport_c::Testserv_Event_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
#include "rosidl_runtime_c/service_type_support_struct.h"
// already included above
// #include "amr_v4_msgs_srvs/srv/detail/testserv__type_support.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/service_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"
#include "service_msgs/msg/service_event_info.h"
#include "builtin_interfaces/msg/time.h"

namespace amr_v4_msgs_srvs
{

namespace srv
{

namespace rosidl_typesupport_c
{
typedef struct _Testserv_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _Testserv_type_support_ids_t;

static const _Testserv_type_support_ids_t _Testserv_service_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _Testserv_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _Testserv_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _Testserv_type_support_symbol_names_t _Testserv_service_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, amr_v4_msgs_srvs, srv, Testserv)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_introspection_c, amr_v4_msgs_srvs, srv, Testserv)),
  }
};

typedef struct _Testserv_type_support_data_t
{
  void * data[2];
} _Testserv_type_support_data_t;

static _Testserv_type_support_data_t _Testserv_service_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _Testserv_service_typesupport_map = {
  2,
  "amr_v4_msgs_srvs",
  &_Testserv_service_typesupport_ids.typesupport_identifier[0],
  &_Testserv_service_typesupport_symbol_names.symbol_name[0],
  &_Testserv_service_typesupport_data.data[0],
};

static const rosidl_service_type_support_t Testserv_service_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_Testserv_service_typesupport_map),
  rosidl_typesupport_c__get_service_typesupport_handle_function,
  &Testserv_Request_message_type_support_handle,
  &Testserv_Response_message_type_support_handle,
  &Testserv_Event_message_type_support_handle,
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_CREATE_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    amr_v4_msgs_srvs,
    srv,
    Testserv
  ),
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_DESTROY_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    amr_v4_msgs_srvs,
    srv,
    Testserv
  ),
  &amr_v4_msgs_srvs__srv__Testserv__get_type_hash,
  &amr_v4_msgs_srvs__srv__Testserv__get_type_description,
  &amr_v4_msgs_srvs__srv__Testserv__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace amr_v4_msgs_srvs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_service_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_c, amr_v4_msgs_srvs, srv, Testserv)() {
  return &::amr_v4_msgs_srvs::srv::rosidl_typesupport_c::Testserv_service_type_support_handle;
}

#ifdef __cplusplus
}
#endif
